...
int main(int argc, char * argv[])
{
    ...
    cxxopts::Options options("PuzzleSolver", std::string("Version: ") + std::string("1") + std::string("\nSolve jigsaw puzzles using the shapes of the piece edges.\n"));
    ...
    timer.stop();
    logger::stream() << std::endl << "time to initialize:"  << timer.getElapsedTime() << std::endl;
    logger::flush();
    timer.start();
    ...
    puzzle.fill_costs();
    timer.stop();
    logger::stream() << std::endl << "time to fill edge costs:"  << timer.getElapsedTime() << std::endl;
    logger::flush();
    timer.start();
    
    puzzle.solve();
    timer.stop();
    logger::stream() << std::endl << "time to solve:"  << timer.getElapsedTime() << std::endl;
    logger::flush();
    timer.start();
    puzzle.save_solution_text();
    puzzle.save_solution_image();
    timer.stop();
    logger::stream() << std::endl << "Time to draw:"  << timer.getElapsedTime() << std::endl;
    logger::flush();
